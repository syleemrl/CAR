  ...
  mTrackCamera(true),  mDrawSim(true), mDrawReg(true), mRD(), mMT(mRD()), mUniform(0.0, 1.0)
{
	...
	mKC = new DPhy::EnemyKinController();
	mKC->Reset();

	std::string path = std::string(CAR_DIR)+std::string("/character/") + std::string(REF_CHARACTER_TYPE) + std::string(".xml");
    mSkel_main = DPhy::SkeletonBuilder::BuildFromFile(path).first;
    mSkel_en = DPhy::SkeletonBuilder::BuildFromFile(path).first;

	DPhy::SetSkeletonColor(mSkel_main, Eigen::Vector4d(235./255., 235./255., 235./255., 1.0));
	DPhy::SetSkeletonColor(mSkel_en, Eigen::Vector4d(235./255., 235./255., 0./255., 1.0));
	Record();

	this->installEventFilter(this);
	setFocusPolicy( Qt::StrongFocus );

	mMotion_main = std::vector<Eigen::VectorXd>();
	mMotion_en = std::vector<Eigen::VectorXd>();

    // std::string path = std::string(CAR_DIR)+std::string("/character/") + std::string(REF_CHARACTER_TYPE) + std::string(".xml");
    DPhy::Character* ref = new DPhy::Character(path);
    DPhy::ReferenceManager* mReferenceManager = new DPhy::ReferenceManager(ref);

    mReferenceManager->LoadMotionFromBVH(std::string("/motion/fight_trajectory.bvh"));
    mTotalFrame = mReferenceManager->GetPhaseLength();
    for(int i = 0; i < mTotalFrame; i++) {
        Eigen::VectorXd p = mReferenceManager->GetPosition(i, false);
        mMotion_main.push_back(p);
    }

    mReferenceManager->LoadMotionFromBVH(std::string("/motion/box_idle.bvh"));
 	std::cout<<"mTotalFrame: "<<mTotalFrame<<std::endl;
    for(int i = 0; i < mTotalFrame; i++) {
        mKC->Step(mMotion_main[i]);
        Eigen::VectorXd p = mKC->GetPosition();
        mMotion_en.push_back(p);
    }
}
void
SceneMotionWidget::
...
SetFrame(int n)
{
	mSkel_main->setPositions(mMotion_main[n]);
	mSkel_en->setPositions(mMotion_en[n]);
...
DrawSkeletons()
{
	GUI::DrawSkeleton(this->mSkel_main, 0);
	GUI::DrawSkeleton(this->mSkel_en, 0);
