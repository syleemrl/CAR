...
Step(dart::simulation::WorldPtr world, Character* character) {
	...
	if(mCurrentFrame < mBlendInterval - 1) {
		...
		std::cout << weight << " / " << mPrevEndPos.segment<6>(0).transpose() <<" / " << mTargetPositions.segment<6>(0).transpose() << std::endl;
	}
	...
	if(mType != "Pivot" && mReferenceManager->GetPhaseLength() <= mCurrentFrame) {
		mEndofMotion = true;
