...
class RegressionNet(object):
	def __init__(self, num_input, num_output):
		self.out = self.create_network(num_input, num_output)

	def create_network(self, num_input, num_output):	
		out = Sequential()
		out.add(Dense(critic_layer_size, activation=activ, input_shape=(num_input, ), dtype=tf.float32))
		out.add(Dense(critic_layer_size, activation=activ, dtype=tf.float32))
		out.add(Dense(num_output, dtype=tf.float32))
