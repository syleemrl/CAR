...
from network3 import Actor
from network3 import Critic
...
class PPO(object):
	...
	def initRun(self, pretrain, num_state, num_action, num_slaves=1):
		...
		save_list = [v for v in tf.compat.v1.trainable_variables() if v.name.find(name)!=-1]
	...
	def createCriticNetwork(self, name, input, TD, clip=True):
		...
		critic_trainer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.learning_rate_critic)
	...
	def buildOptimize(self, name):
		self.state = tf.compat.v1.placeholder(tf.float32, shape=[None, self.num_state], name=name+'_state')
		self.actor = Actor(self.sess, name, self.state, self.num_action)

		with tf.compat.v1.variable_scope(name+'_Optimize'):
			self.action = tf.compat.v1.placeholder(tf.float32, shape=[None,self.num_action], name='action')
			...
			self.old_neglogp = tf.compat.v1.placeholder(tf.float32, shape=[None], name='old_neglogp')
			...
			surrogate = -tf.reduce_mean(input_tensor=tf.minimum(self.ratio*self.GAE, clipped_ratio*self.GAE))
			self.loss_actor = surrogate

		actor_trainer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.learning_rate_ph)
		...
		if self.adaptive:
			if self.parametric:
				self.state_target = tf.compat.v1.placeholder(tf.float32, shape=[None, self.env.dim_param], name=name+'_state_target')

			with tf.compat.v1.variable_scope(name+'_Optimize'):
				if self.parametric:
					self.TD_target = tf.compat.v1.placeholder(tf.float32, shape=[None], name='TD_target')
