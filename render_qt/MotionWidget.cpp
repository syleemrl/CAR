...
getCharacterTransformsForUE(char *buffer)
{
	...
	//this->mSkel = this->mSkel_bvh;
	Eigen::VectorXd pose = this->mSkel->getPositions();

	Eigen::Isometry3d local_space_transform = mController->getLocalSpaceTransform(mSkel);
	...
	for(auto joint_name : this->mJointsUEOrder){
		Eigen::Isometry3d tf;
		if(this->mSkel->getBodyNode(joint_name)->getParentBodyNode() == nullptr) //isRoot : return this->mJointList[jointname]->getParentBodyNode() == nullptr;
			tf = this->mSkel->getBodyNode(joint_name)->getWorldTransform();
			// getBodyGlobalPose : return this->mJointList[bodyname]->getWorldTransform().cast<float>();
		else
		{
			dart::dynamics::BodyNode* body = this->mSkel->getBodyNode(joint_name);
