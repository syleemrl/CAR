  ...
  :MotionWidget()
{
	...
	std::cout<<"total length : "<<mReferenceManager->GetPhaseLength()<<std::endl;
	for(int i=0; i<mReferenceManager->GetPhaseLength(); i++){
		...
		Eigen::Vector3d drawPoint = (lh+rh)/2.;
		// std::cout<<drawPoint.transpose()<<std::endl;
		drawPoint[0] -= 0.75;
		trajPoints.push_back(drawPoint);

		Eigen::Vector3d root = mSkel_bvh->getBodyNode("Hips")->getWorldTransform().translation();
		root[0] -= 0.75;
		trajPoints_root.push_back(root);
	}
...
paintGL()
{
	...
	GUI::DrawTrajectory(trajPoints, trajPoints.size(), Eigen::Vector3d(1, 0.2, 0.2), true, false);
	GUI::DrawTrajectory(trajPoints_root, trajPoints_root.size(), Eigen::Vector3d(0.2, 0.2, 1.0), true, false);
