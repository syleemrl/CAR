...
UpdateParamSpace(std::tuple<std::vector<Eigen::VectorXd>, Eigen::VectorXd, double> candidate) {
	...
	double prev_max = 0;
	bool flag = true;
	double update_max = mUpdateInterval;
	...
	if(flag) {
		std::cout << candidate_scaled.transpose() << " " << std::get<2>(candidate) - prev_max << " "<<update_max << std::endl;
...
GetCPSFromNearestParams(Eigen::VectorXd p_goal) {
	...
	for(int i = 0; i < ps.size(); i++) {
		...
		// std::cout << Denormalize(ps[i].second->param_normalized).transpose() << " " << preward << std::endl;
...
GetFitness(Eigen::VectorXd p) {
	...
	for(int i = 0; i < ps.size(); i++) {
		...
		double fitness = ps[i].second->reward;
		ps_elite.push_back(std::pair<double, Param*>(fitness, ps[i].second));
	}
